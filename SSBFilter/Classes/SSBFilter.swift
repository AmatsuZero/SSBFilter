...
@objcMembers open class SSBBeautyFilter: GPUImageFilter {
    #if targetEnvironment(simulator)
    let kLFGPUImageBeautyFragmentShaderString = """
...
"""
    ...
    public var tone: CGFloat = 0.5
    public var bright: CGFloat = 0.5 {
        didSet {
            setFloat(GLfloat(0.6 * (-0.5 + bright)), forUniformName: "brightness")
        }
    }
    public var beauty: CGFloat = 0.5 {
        didSet { set() }
    }
    
    public override init() {
        super.init(fragmentShaderFrom: kLFGPUImageBeautyFragmentShaderString)
        set()
        bright = 0.5
    }
    
    private func set() {
        var param = GPUVector4()
        param.one = GLfloat(1.0 - 0.6 * beauty)
        param.two = GLfloat(1.0 - 0.3 * beauty)
        param.three = GLfloat(0.1 + 0.3 * tone)
        param.four = GLfloat(0.1 + 0.3 * tone)
        setFloatVec4(param, forUniform: "params")
    }
}

@objcMembers open class SSBEmptyFilter: GPUImageFilter {
    #if targetEnvironment(simulator)
    let kLFGPUImageEmptyFragmentShaderString = """
    varying highp vec2 textureCoordinate;
    ...
    public override init() {
        super.init(fragmentShaderFrom: kLFGPUImageEmptyFragmentShaderString)
    }
}
